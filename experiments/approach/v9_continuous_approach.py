...
# Continuous approach profile. V9 keeps the same distance-scaled speeds as V8,
# but does not stop between YOLO frames. It drives continuously, curves toward
# the target while it remains in view, and stops immediately at the requested
# RealSense distance or on a LiDAR safety event.
...
BASKETBALL_HARD_TURN_ERROR_PIXELS = 230
BASKETBALL_MAX_STEER_REDUCTION = 0.70
BASKETBALL_CONTINUOUS_SAFETY_MARGIN_M = 0.08
...
def approach_basketball(motors, lidar_state, detector):
    """Continuously steer toward the basketball and stop at the target depth."""
    motors.stop()
    last_frame_time = float("-inf")
    last_horizontal_error = 0.0
    current_motion = "stopped"

    print("\nBasketball confirmed. Beginning continuous center-and-approach mode.")

    def apply_search_turn():
        """Rotate continuously toward the side where the target was last seen."""
        nonlocal current_motion
        if last_horizontal_error < 0:
            motors.rotate_left(BASKETBALL_ALIGNMENT_TURN_SPEED)
            current_motion = "search-left"
        else:
            motors.rotate_right(BASKETBALL_ALIGNMENT_TURN_SPEED)
            current_motion = "search-right"

    def apply_forward_steering(speed, horizontal_error):
        """Drive continuously and curve by slowing the inside wheels."""
        nonlocal current_motion
        if abs(horizontal_error) <= BASKETBALL_CENTER_TOLERANCE_PIXELS:
            motors.forward(speed)
            current_motion = f"forward-{speed}"
            return

        normalized_error = clamp(
            abs(horizontal_error) / (CAMERA_WIDTH / 2.0),
            0.0,
            1.0,
        )
        reduction = clamp(
            0.20 + normalized_error * BASKETBALL_MAX_STEER_REDUCTION,
            0.20,
            BASKETBALL_MAX_STEER_REDUCTION,
        )

        outer_speed = speed
        inner_speed = max(4.0, speed * (1.0 - reduction))

        if horizontal_error < 0:
            # Ball is left: slow the left wheels so the rover curves left.
            motors.forward_steered(inner_speed, outer_speed)
            current_motion = f"curve-left-{speed}"
        else:
            # Ball is right: slow the right wheels so the rover curves right.
            motors.forward_steered(outer_speed, inner_speed)
            current_motion = f"curve-right-{speed}"
    ...
    while True:
        ...
        safety_distance = (
            FRONT_EMERGENCY_DISTANCE + BASKETBALL_CONTINUOUS_SAFETY_MARGIN_M
        )
        if front_clearance <= safety_distance:
            motors.stop()
            print(
                f"\nEmergency stop during continuous approach: "
                f"obstacle at {front_clearance:.2f}m."
            )
            return "blocked"

        observation, _frame = detector.snapshot()

        # Do not continue driving indefinitely from an old camera decision.
        if now - observation.frame_time > BASKETBALL_MAX_DETECTION_AGE_SECONDS:
            motors.stop()
            current_motion = "stopped-stale-camera"
        ...
        # Keep the last motor command running between YOLO results. Only change
        # steering/speed when the detector publishes a genuinely new result.
        ...
        if not visible:
            motors.stop()
            current_motion = "stopped-no-target"
            if now - observation.last_seen_time > BASKETBALL_REACQUIRE_SECONDS:
                print("\nBasketball lost. Resuming stepwise exploration.")
                return "lost"
            apply_search_turn()
        ...
        if distance_m is not None and distance_m <= BASKETBALL_STOP_DISTANCE_M:
            motors.stop()
            current_motion = "stopped-at-target"
            ...
            # Already close enough: rotate continuously without advancing until
            # the basketball is centered, then finish on the next camera result.
            if horizontal_error < 0:
                motors.rotate_left(BASKETBALL_ALIGNMENT_TURN_SPEED)
                current_motion = "close-align-left"
            else:
                motors.rotate_right(BASKETBALL_ALIGNMENT_TURN_SPEED)
                current_motion = "close-align-right"
            continue

        if distance_m is None:
            motors.stop()
            current_motion = "stopped-no-depth"
            continue

        if abs(horizontal_error) > BASKETBALL_HARD_TURN_ERROR_PIXELS:
            # A very large error means the ball is near an image edge. Turn in
            # place continuously until the next YOLO result rather than driving
            # forward while the target may be outside the rover's path.
            if horizontal_error < 0:
                motors.rotate_left(BASKETBALL_ALIGNMENT_TURN_SPEED)
                current_motion = "hard-align-left"
            else:
                motors.rotate_right(BASKETBALL_ALIGNMENT_TURN_SPEED)
                current_motion = "hard-align-right"
            continue
        ...
        apply_forward_steering(speed, horizontal_error)
...
class MotorController:
    ...
    def forward_steered(self, left_speed, right_speed):
        """Drive both sides forward at different speeds for smooth steering."""
...
def main():
    ...
    try:
        print("=" * 72)
        print("AACVISION STEPWISE BASKETBALL SEARCH + LIDAR MAPPING V9")
        print("=" * 72)
