...
BASKETBALL_TRACK_HOLD_SECONDS = 2.8
...
def approach_basketball(motors, lidar_state, detector):
    """Continuously steer and drive toward the confirmed basketball.

    The detector, thresholds, confirmation logic, and exploration behavior are
    exactly the V8 versions. This function changes only what happens after V8
    confirms the ball: no timed forward pulses and no stop-after-every-frame.
    """
    motors.stop()
    ...
    print("\nBasketball confirmed. Beginning continuous V8 center-and-approach mode.")
    ...
    while True:
        ...
        # This safety check runs much faster than YOLO and always overrides the
        # camera command. It protects the rover while a CPU inference is running.
        ...
        # A single missed YOLO frame should not make the rover jerk to a stop.
        # Hold the last V8 track briefly, then switch to a continuous slow search.
        track_available = (
            observation.center_x is not None
            and seen_age <= BASKETBALL_TRACK_HOLD_SECONDS
        )

        if not fresh_visible and not track_available:
            close_confirm_count = 0
            if seen_age > BASKETBALL_REACQUIRE_SECONDS:
                motors.stop()
            ...
            if last_horizontal_error < 0:
                ...
                motion_text = "search-left"
            else:
                motors.rotate_right(BASKETBALL_REACQUIRE_TURN_SPEED)
                motion_text = "search-right"

            print(
                f"\rCONTINUOUS V8 APPROACH: temporarily lost, {motion_text:>12s}",
                end="",
                flush=True,
            )
        ...
        # Stop only when the RealSense says the target distance has been reached.
        if distance_m is not None and distance_m <= BASKETBALL_STOP_DISTANCE_M:
            ...
            if abs(horizontal_error) <= BASKETBALL_CAPTURE_CENTER_TOLERANCE_PIXELS:
                ...
                print(
                    "\r"
                    f"CONTINUOUS V8 APPROACH YOLO:{observation.confidence:.2f} "
        ...
        # Very large image error means the basketball is near an edge. Rotate in
        # place continuously until it returns to a reliable forward-driving area.
        if abs(horizontal_error) >= BASKETBALL_EXTREME_ERROR_PIXELS:
            if horizontal_error < 0:
                motors.rotate_left(BASKETBALL_ALIGNMENT_TURN_SPEED)
                motion_text = "turn-left"
            else:
                motors.rotate_right(BASKETBALL_ALIGNMENT_TURN_SPEED)
                motion_text = "turn-right"
        elif distance_m is None:
            # Keep centering continuously, but do not advance without depth.
            if horizontal_error < -BASKETBALL_CENTER_TOLERANCE_PIXELS:
                motors.rotate_left(BASKETBALL_ALIGNMENT_TURN_SPEED)
                motion_text = "depth-wait-left"
            elif horizontal_error > BASKETBALL_CENTER_TOLERANCE_PIXELS:
                motors.rotate_right(BASKETBALL_ALIGNMENT_TURN_SPEED)
                motion_text = "depth-wait-right"
            else:
                motors.stop()
                motion_text = "depth-wait"
        else:
            ...
            # Positive error means the ball is to the right: left wheels run
            # faster and right wheels slower. Negative error does the opposite.
            steer_error = 0.0
            if abs(horizontal_error) > BASKETBALL_STEERING_DEADBAND_PIXELS:
...
def main():
    ...
    try:
        print("=" * 72)
        print("AACVISION STEPWISE BASKETBALL SEARCH + LIDAR MAPPING V8.1 ROBUST DETECTOR")
        print("=" * 72)
        print("Your original travel logic: STOP -> SCAN -> DECIDE -> TURN -> DRIVE.")
        print("Fast orange-circle tracking plus YOLO searches continuously.")
