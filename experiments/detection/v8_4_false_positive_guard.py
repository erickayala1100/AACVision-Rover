...
# V8.4 keeps the same camera model, mapping, and movement behavior, but uses
# stricter evidence and temporal consistency before interrupting exploration.
BASKETBALL_CLASS_NAMES = ("sports ball", "basketball")
BASKETBALL_AI_MIN_CONFIDENCE = 0.16
...
BASKETBALL_MIN_ROUNDNESS = 0.45
BASKETBALL_CONFIRM_SCORE = 0.80
...
class BasketballDetector(threading.Thread):
    ...
    def _publish(self, detection, frame, now):
        if detection is not None:
            ...
            confirmed = (
                self.candidate_streak >= BASKETBALL_CONFIRM_CONSECUTIVE_FRAMES
...
def main():
    ...
    try:
        print("=" * 72)
        print("AACVISION STEPWISE BASKETBALL SEARCH + LIDAR MAPPING V8.4 FALSE-POSITIVE GUARD")
        print("=" * 72)
        print("Your original travel logic: STOP -> SCAN -> DECIDE -> TURN -> DRIVE.")
        print("Stricter multi-frame orange/YOLO confirmation with the same V8.3 movement behavior.")
