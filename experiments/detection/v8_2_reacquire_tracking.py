...
BASKETBALL_MAX_DETECTION_AGE_SECONDS = 0.90
BASKETBALL_REACQUIRE_SECONDS = 12.0
...
# V8 detector and exploration are unchanged. Only the post-detection approach
# is continuous: the rover keeps moving while fresh basketball observations
# steer it. Speed still tapers with distance, and LiDAR is checked continuously.
BASKETBALL_ALIGNMENT_TURN_SPEED = 18
BASKETBALL_REACQUIRE_TURN_SPEED = 14
...
BASKETBALL_EXTREME_ERROR_PIXELS = 220
BASKETBALL_STEERING_DEADBAND_PIXELS = 20
BASKETBALL_MAX_STEERING_DELTA = 8.0
BASKETBALL_STEERING_GAIN = 0.055
BASKETBALL_TRACK_HOLD_SECONDS = 0.0
BASKETBALL_CONTROL_LOOP_SECONDS = 0.02
BASKETBALL_REACQUIRE_CONFIRM_FRAMES = 2
BASKETBALL_REACQUIRE_PULSE_SECONDS = 0.16
BASKETBALL_REACQUIRE_SETTLE_SECONDS = 0.28
BASKETBALL_REACQUIRE_SWITCH_SECONDS = 2.0
BASKETBALL_OFFCENTER_STOP_PIXELS = 110
...
def approach_basketball(motors, lidar_state, detector):
    """Track, reacquire, center, and approach the basketball safely.

    Forward motion is allowed only while a fresh basketball observation is
    available. If the target disappears, the rover stops immediately and uses
    short in-place search pulses. It resumes forward travel only after the
    basketball has been seen in consecutive fresh observations and is centered.
    """
    motors.stop()
    last_printed_frame_time = float("-inf")
    last_horizontal_error = 0.0
    close_confirm_count = 0
    reacquire_confirm_count = 0
    lost_since = None
    search_pulse_count = 0

    print("\nBasketball confirmed. Beginning lock-and-reacquire approach mode.")
    ...
    while True:
        ...
        # LiDAR protection always overrides camera commands.
        ...
        observation_age = now - observation.frame_time
        seen_age = now - observation.last_seen_time
        fresh_visible = (
            observation.visible
            and observation.center_x is not None
            and observation_age <= BASKETBALL_MAX_DETECTION_AGE_SECONDS
            and seen_age <= BASKETBALL_MAX_DETECTION_AGE_SECONDS
        )

        # Never continue forward on an old location estimate.
        if not fresh_visible:
            ...
            if lost_since is None:
                lost_since = now
                search_pulse_count = 0
                print("\nBasketball lost from the current frame. Stopping to reacquire.")

            lost_duration = now - lost_since
            if lost_duration > BASKETBALL_REACQUIRE_SECONDS:
                print("\nBasketball could not be reacquired. Resuming stepwise exploration.")
                return "lost"

            # Search first toward the last known side. If that fails, alternate
            # the search direction every few seconds, but only in short pulses.
            preferred_left = last_horizontal_error < 0
            switch_index = int(lost_duration // BASKETBALL_REACQUIRE_SWITCH_SECONDS)
            turn_left = preferred_left if switch_index % 2 == 0 else not preferred_left

            if turn_left:
                motors.rotate_left(BASKETBALL_REACQUIRE_TURN_SPEED)
                motion_text = "pulse-left"
            else:
                motors.rotate_right(BASKETBALL_REACQUIRE_TURN_SPEED)
                motion_text = "pulse-right"

            time.sleep(BASKETBALL_REACQUIRE_PULSE_SECONDS)
            motors.stop()
            search_pulse_count += 1
            print(
                f"\rREACQUIRING basketball: {motion_text:>11s} "
                f"lost:{lost_duration:4.1f}s pulses:{search_pulse_count:02d}",
                end="",
                flush=True,
            )
            time.sleep(BASKETBALL_REACQUIRE_SETTLE_SECONDS)
            continue
        ...
        # After a loss, require consecutive fresh sightings before moving again.
        if lost_since is not None:
            reacquire_confirm_count += 1
            motors.stop()
            if reacquire_confirm_count < BASKETBALL_REACQUIRE_CONFIRM_FRAMES:
                print(
                    f"\rBasketball reacquired; confirming lock "
                    f"{reacquire_confirm_count}/{BASKETBALL_REACQUIRE_CONFIRM_FRAMES}",
                    end="",
                    flush=True,
                )
                time.sleep(BASKETBALL_CONTROL_LOOP_SECONDS)
                continue
            print("\nBasketball lock restored. Resuming approach.")
            lost_since = None
            reacquire_confirm_count = 0

        # At the target distance, stop and save only when centered.
        if distance_m is not None and distance_m <= BASKETBALL_STOP_DISTANCE_M:
            ...
            if abs(horizontal_error) <= BASKETBALL_CAPTURE_CENTER_TOLERANCE_PIXELS:
                ...
                if close_confirm_count >= BASKETBALL_CLOSE_CONFIRM_FRAMES:
                ...
                if horizontal_error < 0:
                    motors.rotate_left(BASKETBALL_ALIGNMENT_TURN_SPEED)
                    motion_text = "final-turn-left"
                else:
                    motors.rotate_right(BASKETBALL_ALIGNMENT_TURN_SPEED)
                    motion_text = "final-turn-right"
                time.sleep(0.08)
                motors.stop()
            time.sleep(BASKETBALL_CONTROL_LOOP_SECONDS)
            continue
        ...
        # Do not advance while the ball is substantially off center. Rotate in
        # place until the target returns to the central corridor.
        if abs(horizontal_error) >= BASKETBALL_OFFCENTER_STOP_PIXELS:
            if horizontal_error < 0:
                motors.rotate_left(BASKETBALL_ALIGNMENT_TURN_SPEED)
                motion_text = "center-left"
            else:
                motors.rotate_right(BASKETBALL_ALIGNMENT_TURN_SPEED)
                motion_text = "center-right"
        elif distance_m is None:
            motors.stop()
            motion_text = "waiting-depth"
        else:
            if distance_m > BASKETBALL_FAR_DISTANCE_M:
                base_speed = BASKETBALL_APPROACH_FAST_SPEED
            elif distance_m > BASKETBALL_NEAR_DISTANCE_M:
                base_speed = BASKETBALL_APPROACH_MEDIUM_SPEED
            else:
                base_speed = BASKETBALL_APPROACH_NEAR_SPEED

            # Slow down during moderate steering corrections so the ball stays
            # inside the camera view instead of being driven past.
            error_abs = abs(horizontal_error)
            if error_abs > BASKETBALL_CENTER_TOLERANCE_PIXELS:
                base_speed = min(base_speed, BASKETBALL_APPROACH_NEAR_SPEED)

            steer_error = 0.0
            if error_abs > BASKETBALL_STEERING_DEADBAND_PIXELS:
                steer_error = horizontal_error
            steering_delta = clamp(
                steer_error * BASKETBALL_STEERING_GAIN,
                -BASKETBALL_MAX_STEERING_DELTA,
                BASKETBALL_MAX_STEERING_DELTA,
            )
            motors.forward_steer(base_speed, steering_delta)
            if steering_delta < -0.5:
                motion_text = f"curve-left-{base_speed}"
            elif steering_delta > 0.5:
                motion_text = f"curve-right-{base_speed}"
            else:
                motion_text = f"forward-{base_speed}"

        if observation.frame_time > last_printed_frame_time + 1e-6:
            last_printed_frame_time = observation.frame_time
            print(
                "\r"
                f"LOCKED APPROACH conf:{observation.confidence:.2f} "
                f"orange:{observation.orange_fraction:.2f} "
                f"error:{horizontal_error:+5.0f}px depth:{distance_text:>7s} "
                f"motion:{motion_text:>16s}",
                end="",
                flush=True,
            )
...
def main():
    ...
    try:
        print("=" * 72)
        print("AACVISION STEPWISE BASKETBALL SEARCH + LIDAR MAPPING V8.2 LOCK-AND-REACQUIRE")
        print("=" * 72)
        print("Your original travel logic: STOP -> SCAN -> DECIDE -> TURN -> DRIVE.")
        print("Fast orange-circle tracking plus YOLO with stop-and-reacquire target lock.")
